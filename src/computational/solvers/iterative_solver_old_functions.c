...
void iterative_solver_file_handler_multiple(int tot_sub_vol, double complex epsilon, double complex epsilon_ref, double k, double delta_V_vector[], double complex alpha_0[],double k_0, double pi,char multithread, char* G_old_file_name, char* G_sys_file_name, double R[][3]){
				...
				{ 
					...
					if(ig_0 != mm && jg_0 != mm) //using reciprocity, some terms would be calculated twice. The updated condition, each term is calculated one
					{
						for(int i_subG_0 = 0; i_subG_0 < 3; i_subG_0++) // 3D coordinate positions
						{
							...
							double complex G_new_ij_value = G_old_ij_value + G_sys_prod; //alpha_0[mm]*[ig_0_2d][jg_0_2d]
