...
int main() {
    ...
    size_t tot_sub_vol = const_N_subvolumes_per_object *
                         const_N_bulk_objects; // Assign tot_sub_vol: Computes the total number of subvolumes
    ...
    strcpy(discretization_thin_film_file,
           discretization_thin_film); //https://stackoverflow.com/questions/6008733/expression-must-be-a-modifiable-l-value
...
//int (*a)[sz[1]][sz[2]] = calloc(sz[0], sizeof(*a));
    ...
    double (*R)[3] = calloc(tot_sub_vol,
                            sizeof(*R));//double R[tot_sub_vol][3]; // center of subvolumes for thermal objects: info imported from a .txt file
...
//double E_ev[N_omega];
    ...
    double (*delta_V_vector) = malloc(sizeof *delta_V_vector *
                                      tot_sub_vol); //double delta_V_vector[tot_sub_vol]; //Vector of all subvolume size. Combines delta_V_1 and delta_V_2 in the same array
    double (*T_vector) = malloc(sizeof *T_vector *
                                tot_sub_vol); //double T_vector[tot_sub_vol]; // (N x 1) vector of all subvolume temperatures [K]
    double complex(
    *alpha_0) = malloc(
            sizeof *alpha_0 * tot_sub_vol); //  double complex alpha_0[tot_sub_vol]; //Bare polarizability [m^3]
...
//double (*G_12_omega_SGF) = malloc(sizeof *G_12_omega_SGF *N_omega); //double G_12_omega_SGF[N_omega]; //  //double G_12_omega_SGF[tot_sub_vol][tot_sub_vol];
    double (*G_12_omega_SGF)[N_Tcalc] = calloc(N_omega,
                                               sizeof(*G_12_omega_SGF));//double G_12_omega_SGF[N_omega][N_Tcalc]; //

    double (*modulo_r_i_j)[tot_sub_vol] = malloc(
            sizeof *modulo_r_i_j * tot_sub_vol); //double modulo_r_i_j[tot_sub_vol][tot_sub_vol];
    double complex(
    *G_element)[tot_sub_vol] = malloc(
            sizeof *G_element * tot_sub_vol); //double complex G_element[tot_sub_vol][tot_sub_vol];
...
// ############# Folders for results ################
    ...
    if (stat(folder_geometry, &st) ==
        -1) // This condition does not allow that new folders with the same name are created
    {
...
// ######### Properties for thermal objects ###########
    ...
    if (strcmp(geometry, "sphere") == 0) //cannot compare strings in C with ==; source: https://ideone.com/BrFA00
    {
        sprintf(dirPathFileNameDISCRETIZATION, "discretizations/sphere_subvol_%d.txt",
                N_subvolumes_per_object); // path where the file is stored
        import_discretization = fopen(dirPathFileNameDISCRETIZATION, "r"); // "r" = read, "w" = write
        while (3 == fscanf(import_discretization, "%e %e %e", &shape_file[i_import].x, &shape_file[i_import].y,
                           &shape_file[i_import].z)) {
            i_import++;
        }
        double origin1[3] = {radius1, radius1, radius1}; // first index is 0
        double origin2[3] = {origin1[0] + radius1 + d + radius2, origin1[1] + radius2 - radius1,
                             origin1[2] + radius2 - radius1}; // first index is 0
        for (int i_subvol = 0; i_subvol < tot_sub_vol; i_subvol++) //tot_sub_vol
        {
            if (i_subvol < N_subvolumes_per_object) {
                R[i_subvol][0] = shape_file[i_subvol].x * pow(delta_V_1, 1. / 3) +
                                 origin1[0]; // Lindsay's code: R1*pow(delta_V_1,1/3) + origin1[i]
                R[i_subvol][1] = shape_file[i_subvol].y * pow(delta_V_1, 1. / 3) +
                                 origin1[1]; // Lindsay's code: R1*pow(delta_V_1,1/3) + origin1[i]
                R[i_subvol][2] = shape_file[i_subvol].z * pow(delta_V_1, 1. / 3) +
                                 origin1[2]; // Lindsay's code: R1*pow(delta_V_1,1/3) + origin1[i]
                //printf("delta_V_1 = %e\n",pow(delta_V_1,1./3));
                //printf("R[%d] = %e, %e, %e \n",i_subvol,R[i_subvol][0],R[i_subvol][1],R[i_subvol][2]);
            } else {
                R[i_subvol][0] = shape_file[i_subvol - N_subvolumes_per_object].x * pow(delta_V_2, 1. / 3) +
                                 origin2[0]; // Lindsay's code: R2*pow(delta_V_2,1/3) + origin2[i]
                R[i_subvol][1] = shape_file[i_subvol - N_subvolumes_per_object].y * pow(delta_V_2, 1. / 3) +
                                 origin2[1]; // Lindsay's code: R2*pow(delta_V_2,1/3) + origin2[i]
                R[i_subvol][2] = shape_file[i_subvol - N_subvolumes_per_object].z * pow(delta_V_2, 1. / 3) +
                                 origin2[2]; // Lindsay's code: R2*pow(delta_V_2,1/3) + origin2[i]
    ...
    if (strcmp(geometry, "thin-films") == 0) //cannot compare strings in C with ==; source: https://ideone.com/BrFA00
    {
        ...
        import_discretization = fopen(dirPathFileNameDISCRETIZATION, "r"); // "r" = read, "w" = write
        while (3 == fscanf(import_discretization, "%e %e %e", &shape_filetf[i_import].x, &shape_filetf[i_import].y,
                           &shape_filetf[i_import].z)) {
            i_import++;
        }
        for (int i_subvol = 0; i_subvol < tot_sub_vol; i_subvol++) //tot_sub_vol
        {
    ...
    double step_lambda = (final - initial) / (N_omega -
                                              1); // linspace in C: https://stackoverflow.com/questions/60695284/linearly-spaced-array-in-c
    ...
    for (int i_omega = 0; i_omega < omega_range; i_omega++) // Frequency loop
    {
        ...
        if (strcmp(material, "SiO2") == 0) //cannot compare strings in C with ==; source: https://ideone.com/BrFA00
        {
            ...
            for (int i_epsilon = 0; i_epsilon < 3; i_epsilon++) {
                ...
                epsilon_num = 0. + 0. * I;
                epsilon_num =
                        S[i_epsilon] * (1 - epsilon_term1[i_epsilon]) + S[i_epsilon] * epsilon_term2[i_epsilon] * I;
                epsilon_denom = 0.;
            ...
            epsilon = epsilon_inf * (pow(omega_value, 2) - pow(omega_LO, 2) + I * Gamma * omega_value) /
                      (pow(omega_value, 2) - pow(omega_TO, 2) +
                       I * Gamma * omega_value); // omega_TO omega_LO Gamma epsilon_inf
        }
        ...
        double (*r)[tot_sub_vol][3] = calloc(tot_sub_vol,
                                             sizeof(*r)); //double r[tot_sub_vol][tot_sub_vol][3];  // Complete discretized lattice [m]
        double (*abs_r_ij)[tot_sub_vol][3] = calloc(tot_sub_vol,
                                                    sizeof(*abs_r_ij));  // double abs_r_ij[tot_sub_vol][tot_sub_vol][3];
        double (*unit_r_ij)[tot_sub_vol][3] = calloc(tot_sub_vol,
                                                     sizeof(*unit_r_ij));  //double unit_r_ij[tot_sub_vol][tot_sub_vol][3];
        double (*transpose)[tot_sub_vol][3] = calloc(tot_sub_vol,
                                                     sizeof(*transpose)); //double transpose[tot_sub_vol][tot_sub_vol][3];
        double complex(
        *unit_conj_r_ij)[tot_sub_vol][3] = calloc(tot_sub_vol,
                                                  sizeof(*unit_conj_r_ij));  //double complex unit_conj_r_ij[tot_sub_vol][tot_sub_vol][3];
        double complex(
        *r_i_j_outer_r_i_j)[tot_sub_vol][3][3] = calloc(tot_sub_vol,
                                                        sizeof(*r_i_j_outer_r_i_j));  //double complex r_i_j_outer_r_i_j[tot_sub_vol][tot_sub_vol][3][3];
        ...
        for (int i_i = 0; i_i < tot_sub_vol; i_i++) {
            for (int i_j = 0; i_j < tot_sub_vol; i_j++) {
                for (int i_alpha = 0; i_alpha < 3; i_alpha++) {
                    unit_r_ij[i_i][i_j][i_alpha] =
                            r[i_i][i_j][i_alpha] / modulo_r_i_j[i_i][i_j]; // ˆr -- unit distance
                    transpose[i_j][i_i][i_alpha] = r[i_i][i_j][i_alpha] /
                                                   modulo_r_i_j[i_i][i_j]; // https://www.programiz.com/c-programming/examples/matrix-transpose
                    //printf("%d%d: ",i_alpha,i_alpha);//,j_alpha);
                    unit_conj_r_ij[i_i][i_j][i_alpha] = conj(
                            transpose[i_j][i_i][i_alpha]);// r^+ Conjugate transpose unit distance
            ...
            for (int i_j = 0; i_j < tot_sub_vol; i_j++) {
                ...
                for (int i_alpha = 0; i_alpha < 3; i_alpha++) {
                    for (int j_alpha = 0; j_alpha < 3; j_alpha++) {
                        ...
                        r_i_j_outer_r_i_j[i_i][i_j][i_alpha][j_alpha] = unit_r_ij[i_i][i_j][i_alpha] *
                                                                        unit_conj_r_ij[i_i][i_j][j_alpha];//*unit_conj_r_ij[i_i][i_j][j_alpha]; // [j_alpha] outer product
        ...
        double complex(
        *part2ii) = malloc(sizeof *part2ii * tot_sub_vol); //double complex part2ii[tot_sub_vol];
        double complex(
        *part2iiexp) = malloc(sizeof *part2iiexp * tot_sub_vol); //double complex part2iiexp[tot_sub_vol];
        double complex(
        *part3ii) = malloc(sizeof *part3ii * tot_sub_vol); //double complex part3ii[tot_sub_vol];

        //G^0_ij when i!=j:
        double complex(
        *part1ij) = malloc(sizeof *part1ij * tot_sub_vol); //double complex part1ij[tot_sub_vol];
        double complex(
        *part1aij) = malloc(sizeof *part1aij * tot_sub_vol); //double complex part1aij[tot_sub_vol];
        double complex(
        *part1aijexp) = malloc(sizeof *part1aijexp * tot_sub_vol); //double complex part1aijexp[tot_sub_vol];
        double complex(
        *part2ij) = malloc(sizeof *part2ij * tot_sub_vol); //double complex part2ij[tot_sub_vol];
        double complex(
        *part3ij) = malloc(sizeof *part3ij * tot_sub_vol); //double complex part3ij[tot_sub_vol];

        double (*eyeG_0)[3] = calloc(3, sizeof(*eyeG_0)); //double eyeG_0[3][3];
        double (*eyeA)[tot_sub_vol][3][3] = calloc(tot_sub_vol,
                                                   sizeof(*eyeA)); //double eyeA[tot_sub_vol][tot_sub_vol][3][3];

        // Linear system AG=G^0
        double complex(
        *G_0)[tot_sub_vol][3][3] = calloc(tot_sub_vol,
                                          sizeof(*G_0)); //double complex G_0[tot_sub_vol][tot_sub_vol][3][3]; //[tot_sub_vol][tot_sub_vol][3][3]; [size_mat][size_mat]
        double complex(
        *A)[tot_sub_vol][3][3] = calloc(tot_sub_vol, sizeof(*A)); //double complex A[tot_sub_vol][tot_sub_vol][3][3];
                ...
                if (ig_0 != jg_0) // if i!=j:
                {
                    ...
                    for (int i_subG_0 = 0; i_subG_0 < 3; i_subG_0++) // 3D coordinate positions
                    {
                        for (int j_subG_0 = 0; j_subG_0 < 3; j_subG_0++) // 3D coordinate positions
                        {
                            ...
                            G_0[ig_0][jg_0][i_subG_0][j_subG_0] = part1ij[ig_0] *
                                                                  ((part2ij[ig_0] * eyeG_0[i_subG_0][j_subG_0]) -
                                                                   (part3ij[ig_0] *
                                                                    r_i_j_outer_r_i_j[ig_0][jg_0][i_subG_0][j_subG_0]));  //*r_i_j_outer_r_i_j[ig_0][jg_0]
                            //printf("%e + i %e ; ", creal(G_0[ig_0][jg_0][i_subG_0][j_subG_0]),cimag(G_0[ig_0][jg_0][i_subG_0][j_subG_0]));//[i_subG_0][j_subG_0]
                            A[ig_0][jg_0][i_subG_0][j_subG_0] = eyeA[ig_0][jg_0][i_subG_0][j_subG_0] -
                                                                pow(k_0, 2) * alpha_0[ig_0] *
                                                                G_0[ig_0][jg_0][i_subG_0][j_subG_0];
                ...
                for (int jg_0 = 0; jg_0 < tot_sub_vol; jg_0++) //tot_sub_vol
                {
                    if (ig_0 == jg_0) // if i=j:
                    {
                        for (int j_subG_0 = 0; j_subG_0 < 3; j_subG_0++) // 3D coordinate positions
                        {
                            ...
                            G_0[ig_0][jg_0][i_subG_0][j_subG_0] =
                                    eyeG_0[i_subG_0][j_subG_0] * part1ii[ig_0] * (2. * part3ii[ig_0] - 1.);
        ...
        if (save_A_matrix == 'Y' | save_G0_matrix == 'Y' | save_SGF_matrix == 'Y') {
            sprintf(matrices_folder, "%s/matrices_folder",
                    sep_distance_folder); // How to store words in an array in C? https://www.geeksforgeeks.org/how-to-store-words-in-an-array-in-c/
            if (stat(matrices_folder, &st) ==
                -1) // This condition does not allow that new folders with the same name are created
            {
            ...
            sprintf(frequency_folder, "%s/omega_%d", matrices_folder, i_omega +
                                                                      1); // How to store words in an array in C? https://www.geeksforgeeks.org/how-to-store-words-in-an-array-in-c/
            if (stat(frequency_folder, &st) ==
                -1) // This condition does not allow that new folders with the same name are created
            {
            ...
            if (save_A_matrix == 'Y') {
                ...
                Amatrix = fopen(dirPathAFileName,
                                "w");//array[i_omega]"/A_matrix.txt" // EN: https://www.tutorialspoint.com/c_standard_library/c_function_fopen.htm
                for (int ig_0 = 0; ig_0 < tot_sub_vol; ig_0++) //tot_sub_vol
                {
                    for (int i_subG_0 = 0; i_subG_0 < 3; i_subG_0++) // 3D coordinate positions
                    {
                        for (int jg_0 = 0; jg_0 < tot_sub_vol; jg_0++) //tot_sub_vol
                        {
                            for (int j_subG_0 = 0; j_subG_0 < 3; j_subG_0++) // 3D coordinate positions
                            {
                                fprintf(Amatrix, "%e + i %e ; ", creal(A[ig_0][jg_0][i_subG_0][j_subG_0]),
                                        cimag(A[ig_0][jg_0][i_subG_0][j_subG_0])); // PS: note that the function is fprintf not printf
                            }
                        }
                        fprintf(Amatrix, "\n");
                    }
            ...
            if (save_G0_matrix == 'Y') {
                ...
                G0matrix = fopen(dirPathG0FileName,
                                 "w"); // PT: https://terminalroot.com.br/2014/12/linguagem-c-utilizando-as-funcoes-fopen.html
                for (int ig_0 = 0; ig_0 < tot_sub_vol; ig_0++) //tot_sub_vol
                {
                    for (int i_subG_0 = 0; i_subG_0 < 3; i_subG_0++) // 3D coordinate positions
                    {
                        for (int jg_0 = 0; jg_0 < tot_sub_vol; jg_0++) //tot_sub_vol
                        {
                            for (int j_subG_0 = 0; j_subG_0 < 3; j_subG_0++) // 3D coordinate positions
                            {
                                fprintf(G0matrix, "%e + i %e ; ", creal(G_0[ig_0][jg_0][i_subG_0][j_subG_0]),
                                        cimag(G_0[ig_0][jg_0][i_subG_0][j_subG_0]));
                            }
                        }
                        fprintf(G0matrix, "\n");
                    }
        ...
        double complex(
        *G_sys)[tot_sub_vol][3][3] = calloc(tot_sub_vol,
                                            sizeof(*G_sys)); //double complex G_sys[tot_sub_vol][tot_sub_vol][3][3];
        ...
        if (strcmp(solution, "direct") == 0) {
            ...
            double complex(
            *Alapack) = malloc(sizeof *Alapack * lda * n); //double complex Alapack[lda*n];
            double complex(
            *blapack) = malloc(sizeof *blapack * ldb * nrhs); //double complex blapack[ldb*nrhs];
            double complex(
            *work) = malloc(sizeof *work * lwork); //double complex work[lwork];
        ...
        } else //if(strcmp(solution,"iterative")==0)
        {
            ...
            double complex(
            *G_sys_old)[3 * tot_sub_vol] = calloc(3 * tot_sub_vol,
                                                  sizeof(*G_sys_old)); // 2D array, similar to the matlab code
            double complex(
            *G_sys_new)[3 * tot_sub_vol] = calloc(3 * tot_sub_vol,
                                                  sizeof(*G_sys_old)); // 2D array, similar to the matlab code
            double (*eyeA_2d)[3 * tot_sub_vol] = calloc(3 * tot_sub_vol,
                                                        sizeof(*eyeA_2d)); // 2D array, similar to the matlab code
            double complex(
            *A_2d)[3 * tot_sub_vol] = calloc(3 * tot_sub_vol, sizeof(*A_2d)); // 2D array, similar to the matlab code
            double complex(
            *A1lapack) = malloc(sizeof *A1lapack * 3 * 3); //double complex Alapack[lda*n];
            double complex(
            *b1lapack) = malloc(sizeof *b1lapack * 3 * 3); //double complex blapack[ldb*nrhs];

            double complex(
            *epsilon_s) = malloc(sizeof *epsilon_s * tot_sub_vol); //not used
            ...
            for (int mm = 0; mm < tot_sub_vol; mm++) //tot_sub_vol
            {
                ...
                for (int mm_sub = 0; mm_sub < 3; mm_sub++) {
                    ...
                    for (int mm_sub_n = 0; mm_sub_n < 3; mm_sub_n++) {
                        ...
                        //{
                        // First, solve ii = mm system of equations.
                        A_2d[mm_sub][mm_sub_n] = eyeA_2d[mm_2d][mm_2d_n] -
                                                 pow(k, 2) * delta_V_vector[mm] * epsilon_s[mm] *
                                                 G_sys_old[mm_2d][mm_2d_n]; //modification...see if it works
                ...
                for (int jg_0 = 0; jg_0 < tot_sub_vol; jg_0++) // Only loop through remaining perturbations
                {
                    ...
                    info = LAPACKE_zgels(LAPACK_ROW_MAJOR, 'N', 3, 3, 3, A1lapack, 3, b1lapack,
                                         3); //info = LAPACKE_zgels(LAPACK_ROW_MAJOR,'N',m=3,n=3,nrhs=3,Alapack,lda=3,blapack,ldb=3);
                        ...
                        for (int i_subG_0 = 0; i_subG_0 < 3; i_subG_0++) // 3D coordinate positions
                        {
                            ...
                            for (int jg_0 = 0; jg_0 < tot_sub_vol; jg_0++) //tot_sub_vol  ig_0 //lower triangular matrix
                            {
                                ...
                                for (int j_subG_0 = 0; j_subG_0 < 3; j_subG_0++) // 3D coordinate positions
                                {
                                    jg_0_2d = (3 * jg_0 + j_subG_0); // Set indices
                                    mm_2d = (3 * mm + mm_sub);
                                    G_sys_new[ig_0_2d][jg_0_2d] = G_sys_old[ig_0_2d][jg_0_2d] +
                                                                  pow(k, 2) * alpha_0[mm] * G_sys_old[ig_0_2d][mm_2d] *
                                                                  G_sys_new[mm_2d][jg_0_2d]; //alpha_0[mm] = delta_V_vector[mm]*epsilon_s[mm]
                                    mm_sub = mm_sub + 1;
                                } // j_subG_0
                            } // jg_0
                        } // i_subG_0   	
        ...
        if (save_SGF_matrix == 'Y') {
            ...
            sprintf(dirPathG_sys_FileName, "%s/%s", frequency_folder,
                    "G_sys_matrix.txt"); // path where the file is stored
            G_sys_matrix = fopen(dirPathG_sys_FileName,
                                 "w"); // PT: https://terminalroot.com.br/2014/12/linguagem-c-utilizando-as-funcoes-fopen.html
            for (int ig_0 = 0; ig_0 < tot_sub_vol; ig_0++) //tot_sub_vol
            {
                for (int i_subG_0 = 0; i_subG_0 < 3; i_subG_0++) // 3D coordinate positions
                {
                    for (int jg_0 = 0; jg_0 < tot_sub_vol; jg_0++) //tot_sub_vol
                    {
                        for (int j_subG_0 = 0; j_subG_0 < 3; j_subG_0++) // 3D coordinate positions
                        {
                            fprintf(G_sys_matrix, "%e + i %e ; ", creal(G_sys[ig_0][jg_0][i_subG_0][j_subG_0]),
                                    cimag(G_sys[ig_0][jg_0][i_subG_0][j_subG_0]));
                        }
                    }
        ...
        double complex(
        *trans_coeff)[tot_sub_vol] = calloc(tot_sub_vol,
                                            sizeof(*trans_coeff)); //double complex trans_coeff[tot_sub_vol][tot_sub_vol];//[3][3]
        double complex(
        *G_sys_cross)[tot_sub_vol][3][3] = calloc(tot_sub_vol,
                                                  sizeof(*G_sys_cross)); //double complex G_sys_cross[tot_sub_vol][tot_sub_vol][3][3];
        double complex(
        *transpose_G_sys)[tot_sub_vol][3][3] = calloc(tot_sub_vol,
                                                      sizeof(*transpose_G_sys)); //double complex transpose_G_sys[tot_sub_vol][tot_sub_vol][3][3];
            ...
            for (int jg_0 = 0; jg_0 < tot_sub_vol; jg_0++) //tot_sub_vol
            {
                for (int i_subG_0 = 0; i_subG_0 < 3; i_subG_0++) // 3D coordinate positions
                {
                    for (int j_subG_0 = 0; j_subG_0 < 3; j_subG_0++) // 3D coordinate positions
                    {
                        ...
                        G_sys_cross[ig_0][jg_0][i_subG_0][j_subG_0] = conj(
                                transpose_G_sys[jg_0][ig_0][i_subG_0][j_subG_0]); // G_element'
                        //if (i_subG_0==i_subG_0) //
                        //{
                        G_element[ig_0][jg_0] += G_sys[ig_0][jg_0][i_subG_0][j_subG_0] *
                                                 G_sys_cross[ig_0][jg_0][i_subG_0][j_subG_0]; //sum(diag((G_element*(G_element'))
                        //}
                    }
                }
                ...
                trans_coeff[ig_0][jg_0] =
                        4. * pow(k_0, 4) * delta_V_vector[ig_0] * delta_V_vector[jg_0] * cimag(epsilon) *
                        cimag(epsilon) * G_element[ig_0][jg_0];
        ...
        if (save_spectral_transmissivity == 'Y') {
...

            {
                ...
                sprintf(dirPathSpectral_trans_FileName, "%s/%d.csv", spectral_transmissivity_folder,
                        i_omega + 1); // path where the file is stored
        ...
        double (*dtheta_dT) = malloc(sizeof *dtheta_dT *
                                     N_Tcalc); //double dtheta_dT[N_Tcalc]; // function used to calculate conductance, modified for several temperatures
            ...
            if (save_spectral_conductance == 'Y') {
                {
                    ...
                    sprintf(dirPathSpectral_cond_FileName, "%s/d_%e/spectral_conductance_%eK.csv", folder_subvol, d,
                            Tcalc_vector[iTcalc]); // path where the file is stored
    ...
    */
    ...
    sprintf(dirPathpos_processing_summary_FileName, "%s/pos_processing_summary.txt",
            sep_distance_folder); // path where the file is stored

    pos_processing_summary = fopen(dirPathpos_processing_summary_FileName, "w");
    fprintf(pos_processing_summary,
    ...
    fopen(dirPathpos_processing_summary_FileName,
          "a"); // Opens file for appending. (File remains unchanged, file pointer gets moved to end) https://stackoverflow.com/questions/16427664/trying-not-to-overwrite-a-file-in-c/16427698
    fprintf(pos_processing_summary, "Time counter: %f s\n", time_spent);
    fclose(pos_processing_summary);
